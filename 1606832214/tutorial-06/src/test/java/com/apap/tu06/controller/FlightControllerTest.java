...
public class FlightControllerTest {
	...
	public void givenFlightNumber_whenViewFlight_thenReturnJsonFlight() throws Exception {
		...
		// When Then
		mvc.perform(MockMvcRequestBuilders.get("/flight/view").param("flightNumber", flight.get().getFlightNumber()).contentType(MediaType.APPLICATION_JSON)).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.jsonPath("$.flightNumber", Matchers.is(flight.get().getFlightNumber())));
	}
}
