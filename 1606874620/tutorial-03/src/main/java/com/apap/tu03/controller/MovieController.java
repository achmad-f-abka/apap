...
public class MovieController {
	...
	public String durationUpdate(@PathVariable Optional<String> id, @PathVariable Optional<Integer> duration, Model model) {
		if(id.isPresent()) {
			for(MovieModel movie: movieService.getMovieList()) {
				if(movie.getId().equals(id.get())) {
	...
	public String deleteMovie(@PathVariable Optional<String> id, Model model) {
		if(id.isPresent()) {
			for(MovieModel movie: movieService.getMovieList()) {
				if(movie.getId().equals(id.get())) {
					for (Iterator<MovieModel> iterateMov = movieService.getMovieList().listIterator(); 
							iterateMov.hasNext();) {
						
					    MovieModel a = iterateMov.next();
