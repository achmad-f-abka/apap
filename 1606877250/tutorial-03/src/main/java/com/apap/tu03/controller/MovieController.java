...
public class MovieController {
	...
	public String viewMovie(@PathVariable String id, Model model) {
		...
		for (int i = 0; i <= archive.size()-1; i++) {
			if (archive.get(i).getId().equalsIgnoreCase(id)) {
				MovieModel archive2 = movieService.getMovieDetail(id);
				model.addAttribute("movie", archive2);
				return "view-movie";
			}
		}
		return "error";
	...
	public String updateMovie(@PathVariable String id, @PathVariable Integer duration, Model model) {
		...
		for (int i = 0; i <= archive.size()-1; i++) {
			if (archive.get(i).getId().equalsIgnoreCase(id)) {
				Integer oldDuration = archive.get(i).getDuration();
				
				MovieModel archive2 = movieService.getMovieDetail(id);
				archive2.setDuration(duration);
				Integer newDuration = archive2.getDuration();
				model.addAttribute("movie", archive2);
	...
	public String deleteMovie(@PathVariable String id, Model model) {
		...
		for (int i = 0; i <= archive.size()-1; i++) {
			if (archive.get(i).getId().equalsIgnoreCase(id)) {
				String judul = archive.get(i).getTitle();
