...
public class PageController {
	...
	/*
	 * @RequestMapping(value= {"/hello2","/hello2/{name}"}) public String
	 * helloPath(@PathVariable Optional<String> name, Model model) {
	 * if(name.isPresent()) { model.addAttribute("name",name.get() ); }else {
	 * model.addAttribute("name","Phoenix"); } return "hello2"; }
	 */
	
	@RequestMapping("/calculator")
    public String jumlah(@RequestParam(value = "x", required = false, defaultValue = "2") int x,
                            @RequestParam(value = "y", required = false, defaultValue = "9") int y, 
                            Model model) {
        ...
        if (result < 9999 && result > 0) {
            ...
            modRibuan= result / 1000;
            modRatusan= (result % 1000) / 100;
            modPuluhan= (result % 100) / 10;
            modDozen= result % 10;
            if (modPuluhan== 1) {
                if (modDozen== 1) {
