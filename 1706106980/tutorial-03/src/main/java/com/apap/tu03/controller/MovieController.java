...
public class MovieController {
		...
		if(id.isPresent()) {
			for(MovieModel movie: movieService.getMovieList()) {
				if(movie.getId().equals(id.get())) {
					...
					int newDurationOfMovie = duration.get();
					model.addAttribute("title", TitleOfMovie);
					model.addAttribute("movieDuration", DurationOfMovie);
					model.addAttribute("newMovieDuration", newDurationOfMovie);
	...
	public String deleteMovie(@PathVariable Optional<String> id, Model model) {
		if(id.isPresent()) {
			for(MovieModel movie: movieService.getMovieList()) {
				if(movie.getId().equals(id.get())) {
					for (Iterator<MovieModel> iteration = movieService.getMovieList().listIterator(); iteration.hasNext(); ) {
					    MovieModel a = iteration.next();
